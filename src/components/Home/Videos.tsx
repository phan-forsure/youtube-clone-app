        ...
        <section className="videos text-white grid grid-cols-4 max-xl:grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1">
            {query.data?.data?.map(item => (
                <div key={item.videoId} className="data-unit m-2 relative bottom-0 hover:bottom-1 transition-all overflow-hidden">
                    <Link replace to={`/video/${item.videoId}`}><img key={item.videoId} className='cursor-pointer rounded-2xl w-full' src={item.thumbnail[2]?.url !== undefined ? item.thumbnail[2]?.url : item.thumbnail[1]?.url !== undefined ? item.thumbnail[1]?.url : item.thumbnail[0]?.url} alt="Thumbnail" /></Link>
